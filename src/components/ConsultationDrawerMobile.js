...
import { errorMessage, getClinicName, trackEvent, getTokenData, getDeviceSdkData, getVoiceRxMoengageBasePayload, isVoiceRxFree, getSupportedMimeType, removeBeforeWhiteSpace, getHmTypeIndicator, getClinic, stopAllActiveRecorders } from "../utils/utils";
...
const ConsultationDrawerMobile = ({
  visible,
  onClose,
  labReportID,
  digitizedData,
  ...
  const { patient_data, caseManagerData, fromVoiceRecording, audioBlob: audioBlobFromLocation, consent, noStayClickCount, crossButtonClickCount, mutePauseButtonClickCount } = state || {};
  ...
  useEffect(() => {
    if (
      ...
      window.Moengage?.track_event(
        targetMode === "ambient"
          ? "TP_App_AV_SC_Autofill_Ambient"
          : "TP_App_AV_SC_Autofill_Voice",
        {
    ...
    try {
      ...
      if (initialResponse.success && initialResponse.data) {
        ...
        if (response.success && payload) {
          ...
          // Prescription sources (priority): editedData → digitizeData → refinedData → history[0].digitize
          ...
          if (prescriptionData) {
            ...
            if (validatedPrescriptionData) {
              setPrescriptionData((prev) => {
                const next = validatedPrescriptionData;
                const labResults = (Array.isArray(next?.labResults) && next.labResults.length > 0)
                  ? next.labResults
                  : (Array.isArray(prev?.labResults) && prev.labResults.length > 0 ? prev.labResults : []);
                return { ...next, labResults };
  ...
  const handleStopRecording = () => {
    ...
    const clinic_name = getClinicName(profile?.hospital_data);
    trackEvent("TP_App_VoiceRx_Paused", {
  ...
  const handleSend = async () => {
    window.Moengage.track_event("TP_App_AV_TypeTranscript", {
      ...getVoiceRxMoengageBasePayload({
        profile,
        userId,
        patientData: patient_data,
  ...
  const validatePrescriptionData = (data) => {
    ...
    const CANONICAL_VITAL_KEYS = ['temperature', 'pulse', 'respRate', 'bloodPressure', 'systolic', 'diastolic', 'spo2', 'ofc', 'height', 'weight', 'BMI', 'BMR', 'BSA', 'general_rbs', 'fib4', 'waist_circumference'];
    const normalizeVitalsForPrescription = (vitalsAndBodyComposition) => {
      ...
      if (generalRBSVal !== undefined && generalRBSVal !== null && String(generalRBSVal).trim() !== '') {
        result.general_rbs = String(generalRBSVal).trim();
      }
      return result;
    };
    ...
    const mappedLabInvestigation = Array.isArray(cleanedData.labInvestigation)
      ? cleanedData.labInvestigation.map(item => {
          ...
          const mappedItem = {
            ...restItem,
            notes: item.lineItem || item.instruction || item.notes || ''
          };
  ...
  const hasRxPadContent = (data) => {
    ...
    if (hasArray(data.symptoms) || hasArray(data.medications) || hasArray(data.medicalHistory) || hasArray(data.diagnosis) || hasArray(data.labResults) || hasArray(data.vaccinations)) return true;
  ...
  const mapPrescriptionDataToDigitizeData = (prescriptionData) => {
    ...
    const v = prescriptionData.vitalsAndBodyComposition;
    if (!v || typeof v !== "object") return { ...prescriptionData };
    const mapped = { ...v };
    ...
    return { ...prescriptionData, vitalsAndBodyComposition: mapped };
  };
  ...
  const handleVoiceDigitize = async (
    ...
    const modeForEvent =
      ...
      genRxDetails?.type === "ambient"
        ? "ambient"
        : "dictation";
    const submitId = `mobile-voice-submit-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
    const requestId = submitId;
    const voiceApiStartMs = Date.now();
    ...
    if (!isFromSC) {
      trackEvent("TP_App_Voice_Submit_Click", {
        ...
        micBeingUsed: audioStreamRef.current?.getAudioTracks?.()?.[0]?.label || "Unknown",
        browser: "mobile_web",
        durationOfAudioInSeconds: recordingTime || 0,
        voiceApiCalled: true,
      ...
      if (isAmbientMode || mode === 'ambient' || window.TATVA_ACTIVE_VOICE_SERVICE === S_AMBIENT_VOICE_RX || genRxDetails?.type === 'ambient') {
        ...
        if (isSuccess) {
          ...
          if (!isFromSC) {
            ...
            // Rx pad: always display from digitizeData when present (voice/ambient); fallback to editedData only when no digitizeData
            ...
            if (prescriptionData && prescriptionData.vitalsAndBodyComposition) {
              const vitals = prescriptionData.vitalsAndBodyComposition;
              const actualVitals = vitals._doc || vitals;
              const cleanedVitals = {};
              Object.entries(actualVitals).forEach(([key, value]) => {
                if (value !== undefined && value !== null && String(value).trim() !== '') {
                  cleanedVitals[key] = value;
                }
              });
              prescriptionData.vitalsAndBodyComposition = cleanedVitals;
            }

            if (!conversationData) {
              if (actualData?.rxDigitizationHistory?.[0]?.payload?.conversation) {
                conversationData = actualData.rxDigitizationHistory[0].payload.conversation;
              ...
              } else if (actualData?.conversation) {
                conversationData = actualData.conversation;
              } else if (responseData?.conversation) {
                conversationData = responseData.conversation;
              } else if (actualData?.digitize?.conversation) {
                conversationData = actualData.digitize.conversation;
              } else if (responseData?.digitize?.conversation) {
                conversationData = responseData.digitize.conversation;
              } else if (actualData?.rx?.conversation) {
                conversationData = actualData.rx.conversation;
              } else if (responseData?.rx?.conversation) {
                conversationData = responseData.rx.conversation;
              } else if (prescriptionData?.conversation) {
                conversationData = prescriptionData.conversation;
              }
            }
        ...
        if (isSuccess) {
          if (!isFromSC) {
            const moduleSummary = getFilledModulesSummary(dataForSuccess || {});
            trackEvent("TP_App_Voice_Api_Result", {
              ...getVoiceRxMoengageBasePayload({
                profile,
                userId,
              ...
              prescriptionUrl: "",
              _id: dataForSuccess?._id || responseData?._id || "",
              voice_failed: false,
              error_message: "",
              retry_attempts: 0,
              isemptyprescription: moduleSummary.rxModulesFilledCount === 0,
          ...
          // Rx pad: use editedData when present, else digitizeData, else refinedData
          ...
          const dataForPadDictate = editedDataDictate ?? digitizeDataDictate ?? dataForParse.refinedData ?? null;
          ...
          if (!isFromSC) {
            ...
            if (dataForPadDictate) {
              prescriptionData = editedDataDictate
                ...
                : mapDigitizeDataToPrescription(digitizeDataDictate ?? dataForParse.refinedData);
            }
            ...
            if (prescriptionData) {
              ...
              if (validatedPrescriptionDataForDictate) {
                ...
                setPrescriptionData((prev) => {
                  return { ...validatedPrescriptionDataForDictate, labResults: labResultsToUse };
                });   
  ...
  const handleUpdateGenRX = async (
    ...
    const modeForEvent =
      ...
      genRxDetails?.type === "ambient"
        ? "ambient"
        : "dictation";
    const submitId = `mobile-voice-update-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
    const requestId = submitId;
    const voiceApiStartMs = Date.now();
    try {
      let response;
      ...
      // Do not send tcv_id, tcbc_id, dev_unique_id in patch payload; normalize general_rbs -> generalRBS for API
  ...
  async function onEndVisitClick() {
    ...
    window.Moengage.track_event("TP_App_AV_SaveRx", {
      ...getVoiceRxMoengageBasePayload({
        profile,
        userId,
        patientData: patient_data,
    ...
    if (action.meta.requestStatus === "fulfilled") {
      ...
      const clinic_name = getClinicName(profile?.hospital_data);
      const deviceSdkData = getDeviceSdkData();
      window.Moengage.track_event("TP_App_Voice_Submit", {
        doctor_name: profile?.um_name,
        doctor_number: profile?.um_contact,
        doctor_unique_id: profile?.doctor_unique_id,
        doctor_specialty: profile?.dp_name,
      ...
      if (isGroundingAccessableForZydus && patient_data?.departmentId) {
        ...
        if (actionIM.meta.requestStatus === "fulfilled") {
          let zydusSendData = {
            ...
            duplicateCheck: 1,
            investigationList:
              prescriptionData?.labInvestigation?.map((item) => item.name) ||
              [],
            medicineList:
  ...
  const checkDataFillOrNot = () => {
    ...
    if (prescriptionData) {
      ...
      if (isAmbientMode || mode === 'ambient' || window.TATVA_ACTIVE_VOICE_SERVICE === S_AMBIENT_VOICE_RX) {
        ...
        navigateVoiceRx({ patient_data }, { replace: true }, "mobile_consultation_drawer_ambient_back");
        onClose();
    } else {
      onClose();
      }
  ...
  const handleBack = () => {
    ...
    navigateVoiceRx({ patient_data }, { replace: true }, "mobile_consultation_drawer_back");
  };
                  ...
                  >
                      <div className={styles.chatSection}>
                        ...
                        {!isFirstTranscriptJSON && !isProcessing && ((Array.isArray(queries) && queries.length > 0) || !!fullTranscript || !!genRxDetails?.source || (isAmbientMode && conversations?.length > 0)) && (
                            <div className="d-flex align-items-start gap-2 mt-3" style={{ padding: '0 5px' }}>
                      ...
                      <div className="d-flex align-items-center mt-2 justify-content-end">
                        <div
                          onClick={() => {
                            ...
                            if (isAmbientMode || mode === 'ambient' || window.TATVA_ACTIVE_VOICE_SERVICE === S_AMBIENT_VOICE_RX) {
                              // Return to the correct VoiceRx entry point for this feature flag state.
                              navigateVoiceRx({ patient_data }, { replace: true }, "mobile_consultation_drawer_leave_ambient");
                            } else {
...
export default ConsultationDrawerMobile;
